...
def train():
	...
	for i in training_data.split(" "):
		data_to_append = generated_data["dialogue" if is_dialogue else "story"]

		if last_word == "":
			data_to_append["start"].append(i)
		else:
			if last_word in data_to_append["tokens"]:
				data_to_append["tokens"][last_word].append(i)
			elif len(last_word) > 1 or last_word.isalpha():
				data_to_append["tokens"][last_word] = [i]
...
def generate_text():
	text = random.choice(generated_data["story"]["start"])
	last_word = text
	is_dialogue = False

	for i in range(400):
		if '"' in last_word and not last_word.endswith('"'):
			is_dialogue = last_word.startswith('"')
			last_word = ""

		data_to_use = generated_data["dialogue" if is_dialogue else "story"]

		if last_word in data_to_use["tokens"]:
			last_word = random.choice(data_to_use["tokens"][last_word])

			text += " " + last_word
		else:
			last_word = random.choice(data_to_use["start"])

		if '"' in last_word and not last_word.startswith('"'):
			is_dialogue = last_word.endswith('"')
			last_word = ""
